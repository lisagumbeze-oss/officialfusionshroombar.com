...
async function seed() {
    process.stdout.write("Seeding interactions...\n");
    ...
    for (const post of blogPosts) {
        ...
        const shuffledTemplates = blogCommentTemplates.slice().sort(() => 0.5 - Math.random());
        const selectedTemplates = shuffledTemplates.slice(0, numComments);
    ...
    process.stdout.write(`Seeded comments for ${blogPosts.length} blog posts.\n`);

    for (const product of products) {
        const numReviews = 5 + Math.floor(Math.random() * 6); // 5-10 reviews
        const shuffledTemplates = productReviewTemplates.slice().sort(() => 0.5 - Math.random());
    ...
    process.stdout.write(`Seeded reviews for ${products.length} products.\n`);
